...
void main() {
  ...
  test('accepts any readable Google Sheet returned by Drive', () {
  ...
  test('builds Sheet history from accessible Drive metadata', () {
    expect(
    ...
    expect(recent.first.id, 'owned-sheet-id');
    expect(recent.first.modifiedAt, modifiedAt);
    expect(recent.first.createdAt, createdAt);
    expect(
      recent.first.url,
      'https://docs.google.com/spreadsheets/d/owned-sheet-id/edit',
    );
    expect(recent.last.id, 'shared-sheet-id');
  });
